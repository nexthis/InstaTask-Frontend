...
import { getURLParameter, combineURLs } from 'utils/url'

export async function login(): Promise<(UserInterface | null)> {

    const win = window.open(combineURLs(process.env.REACT_APP_AUTH_URL as string, 'instagram'), "_blank", 'toolbar=0,location=yes,scrollbars=1,statusbar=1,menubar=0,resizable=1,width=500,height=500,left=50,top=50')
